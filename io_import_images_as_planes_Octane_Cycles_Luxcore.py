...
bl_info = {
    "name": "Import Images as Planes - Cycles, Eevee, Luxcore Octane Support",
    "author": "Florian Meyer (tstscr), mont29, matali, Ted Schundler (SpkyElctrc), Ed O'Connell, Adam Zorian Radziszewski",
    "version": (3, 4, 0),
    "blender": (2, 93, 0),
    "location": "File > Import > Images as Planes or Add > Mesh > Images as Planes",
    "description": "Imports images and creates planes with the appropriate aspect ratio. "
                   "The images are mapped to the planes."
                   "Octane Support was made by Ed O'Connell. "
                   "Luxcore Support by Adam Zorian Radziszewski",
    "warning": "",
...
def auto_align_nodes(node_tree):
    ...
    output_node = None
    for node in nodes:
        if node.type == 'OUTPUT_MATERIAL' or node.type == 'GROUP_OUTPUT' or node.bl_idname =='LuxCoreNodeMatOutput':
            output_node = node
            break
...
def clean_luxcore_tree(node_tree):
    """Clear all nodes in a shader node tree except the output.

    Returns the output node
    """
    nodes = luxcore.node_tree.nodes
    for node in list(nodes):
        if not node.bl_idname == 'LuxCoreNodeMatOutput':
            nodes.remove(node)
                
    return luxcore.node_tree.nodes[0]
...
class IMPORT_IMAGE_OT_to_plane(Operator, AddObjectHelper):
    ...
    OCTSHADERS = (
        ('PRINCIPLED',"Universal","Universal Material"),
        ('EMISSION', "Emit", "Emission Shader"),
    )
    
    #shader: EnumProperty(name="Shader", items=SHADERS, default='PRINCIPLED', description="Node shader to use")
    octshader: EnumProperty(name="Shader", items=OCTSHADERS, default='PRINCIPLED', description="Node shader to use")
    
    LUXSHADERS = (
        ('PRINCIPLED', 'Disney', 'Disney Luxcore Shader'),
        ('EMISSION', 'Emission', 'Emission Luxcore Shader'),
    )

    
    luxshader: EnumProperty(name="Shader", items=LUXSHADERS, default='PRINCIPLED', description = "Node Shader to use")
    ...
    emit_doublesided: BoolProperty(
        name="Double-Sided Emission", default=True,
        description="Use Double-sided Emission (Octane only)")
    ...
    def draw_material_config(self, context):
        ...
        if context.scene.render.engine == 'octane':
            row.prop(self, 'octshader', expand=True)
        if context.scene.render.engine == 'LUXCORE':
            row.prop(self, 'luxshader', expand=True)
        ...
        if self.octshader == 'EMISSION':
            box.prop(self, "emit_strength")
            box.prop(self, "emit_doublesided")
        if self.luxshader == 'EMISSION':
            box.prop(self, 'emit_strength')

        engine = context.scene.render.engine
        print (engine)
        if engine not in ('CYCLES', 'octane', 'BLENDER_EEVEE', 'BLENDER_WORKBENCH', "LUXCORE"):
    ...
    def invoke(self, context, event):
        engine = context.scene.render.engine
        if engine not in {'CYCLES', 'BLENDER_EEVEE', 'octane', 'LUXCORE'}:
    ...
    def single_image_spec_to_plane(self, context, img_spec):
        ...
        if engine == 'octane':
            material = self.create_octane_material(context, img_spec)
            
        if engine == 'LUXCORE':
            material = self.create_luxcore_material(context, img_spec)
    ...
    # Luxcore
    def create_luxcore_texnode(self, context, node_tree, img_spec):
        #luxcore_nodes = bpy.data.active_material.luxcore.node_tree.nodes
        tex_image = node_tree.nodes.new('LuxCoreNodeTexImagemap')
        tex_image.image = img_spec.image
        tex_image.show_texture = True
        self.apply_texture_options(tex_image, img_spec)
        return tex_image
    ...
    # Octane
    def create_octane_texnode(self, context, node_tree, img_spec):
        tex_image = node_tree.nodes.new('ShaderNodeOctImageTex')
        tex_image.image = img_spec.image
        tex_image.show_texture = True
        self.apply_texture_options(tex_image, img_spec)
        return tex_image

    def create_octane_alpha_texnode(self, context, node_tree, img_spec):
        tex_image = node_tree.nodes.new('ShaderNodeOctAlphaImageTex')
        tex_image.image = img_spec.image
        tex_image.show_texture = True
        tex_image.inputs['Gamma'].default_value = 1
        self.apply_texture_options(tex_image, img_spec)
        return tex_image
    ...
    def create_cycles_material(self, context, img_spec):
        ...
        auto_align_nodes(node_tree)
        return material
    def create_octane_material(self, context, img_spec):
        image = img_spec.image
        name_compat = bpy.path.display_name_from_filepath(image.filepath)
        material = None
        if self.overwrite_material:
        ...
        tex_image = self.create_octane_texnode(context, node_tree, img_spec)

        if self.octshader == 'PRINCIPLED':
            core_shader = node_tree.nodes.new('OctaneUniversalMaterial')
            core_shader.inputs['Roughness'].default_value = 0.5
        elif self.octshader == 'EMISSION':
            core_shader = node_tree.nodes.new('OctaneDiffuseMaterial')
            core_shader.inputs[0].default_value = (0, 0, 0)
            emission_shader = node_tree.nodes.new('OctaneTextureEmission')
            emission_shader.inputs['Power'].default_value = self.emit_strength
            emission_shader.inputs['Surface brightness'].default_value = True
            emission_shader.inputs['Double sided'].default_value = self.emit_doublesided


        # Connect color from texture
        if self.octshader == 'PRINCIPLED':
            node_tree.links.new(core_shader.inputs['Albedo'], tex_image.outputs[0])
        elif self.octshader == 'EMISSION':
            node_tree.links.new(core_shader.inputs['Emission'], emission_shader.outputs[0])
            node_tree.links.new(emission_shader.inputs[0], tex_image.outputs[0])

        if self.use_transparency:
            alpha_tex_image = self.create_octane_alpha_texnode(context, node_tree, img_spec)
            node_tree.links.new(core_shader.inputs['Opacity'], alpha_tex_image.outputs[0])
    ...
    def create_luxcore_material(self, context, img_spec):
        image = img_spec.image
        name_compat = bpy.path.display_name_from_filepath(image.filepath)
        material = None
        if self.overwrite_material:
        ...
        if not material:
            material = bpy.data.materials.new(name=name_compat)
            material_group = bpy.data.node_groups.new(name= name_compat, type="luxcore_material_nodes")
            material.luxcore.node_tree = material_group
            material.luxcore.node_tree.nodes.new("LuxCoreNodeMatOutput")
        ...
        for node in node_tree.nodes:
            if "LuxCoreNodeMatOutput" in node.bl_idname:
                
                out_node = node

        tex_image = self.create_luxcore_texnode(context, node_tree, img_spec)

        if self.luxshader == 'PRINCIPLED':
            core_shader = node_tree.nodes.new('LuxCoreNodeMatDisney')

            core_shader.inputs['Roughness'].default_value = 0.5
        elif self.luxshader == 'EMISSION':
            core_shader = node_tree.nodes.new('LuxCoreNodeMatMatte')

            core_shader.inputs[0].default_value = (0, 0, 0)
            emission_shader = node_tree.nodes.new('LuxCoreNodeMatEmission')

            emission_shader.gain = 1
            #emission_shader.inputs['Surface brightness'].default_value = True
            #emission_shader.inputs['Double sided'].default_value = self.emit_doublesided
        ...
        # Connect color from texture
        if self.luxshader == 'PRINCIPLED':
            node_tree.links.new(core_shader.inputs['Base Color'], tex_image.outputs[0])
        elif self.luxshader == 'EMISSION':
            node_tree.links.new(core_shader.inputs['Emission'], emission_shader.outputs[0])
            node_tree.links.new(emission_shader.inputs[0], tex_image.outputs[0])
        ...
        if self.use_transparency:
            #alpha_tex_image = self.create_octane_alpha_texnode(context, node_tree, img_spec)
            node_tree.links.new(core_shader.inputs['Opacity'], tex_image.outputs[1])

        node_tree.links.new(out_node.inputs['Material'], core_shader.outputs[0])
